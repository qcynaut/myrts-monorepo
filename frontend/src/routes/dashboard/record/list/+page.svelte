<script lang="ts">
	...
	import { deleteRecords, getRecords } from '$lib/service/record';
	...
	const load = async () => {
		...
		records = res.result?.filter((r) => r.status == 1) || [];
	};

	const downloadAudio = async (url: string, name: string) => {
		try {
	...
	const removeRecord = async () => {
		if (deleteId != null && records[deleteId]) {
			...
			setLoading(false);
			if (res.error) {
				toastError('Gagal menghapus rekaman');
			} else {
				records = records.filter((r) => r.id != record.id);
				toastSuccess('Rekaman berhasil dihapus');
			}
		}
	};
...
{#if deleteId != null}
	<Modal title="Delete rekaman" on:close={() => (deleteId = null)} open>
		<p>Apakah anda yakin ingin menghapus rekaman ini?</p>
		<p class="mt-4 text-gray-500">
			Catatan: Apabila rekaman ini di gunakan di jadwal, harap menghapus jadwal terlebih dahulu.
		</p>
		<div class="mt-4 flex justify-end gap-4">
			<Button on:click={() => (deleteId = null)}>Batal</Button>
			<Button color="red" on:click={removeRecord}>Hapus</Button>
		</div>
	</Modal>
{/if}

<Layout title="Rekaman" subtitle="Manage Rekaman">
	<div class="absolute w-full left-0 bottom-0 z-10 {playId != null ? '' : 'hidden'}">
		<AudioPlayer src={playId != null ? records[playId].file_url : ''}>
			<LargePlayer
				bind:seek
				bind:playerPaused={paused}
				{prev}
				{next}
				close={() => (playId = null)}
			/>
			...
			<TableBody>
				{#each filtered as record, i}
					<TableBodyRow class={playId == getIndex(record) ? 'bg-primary-300' : ''}>
						...
						<TableBodyCell class="whitespace-pre-line">{record.description ?? ''}</TableBodyCell>
						<TableBodyCell>
							<div class="flex">
								<button
									on:click={() => downloadAudio(record.file_url, record.name)}
									class="w-6 h-6 flex justify-center items-center rounded-full bg-primary-500"
								>
									<Icons name={Icon.Download} class="text-white" />
								</button>
